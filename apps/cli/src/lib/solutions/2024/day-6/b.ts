...
export default function (input: string): string | number {
  ...
  /*
    NOTES:
    ref: https://x.com/PaulBN/status/1865202477533893047/photo/1
    for each position in visited rotate right (to simulate a blockage) then run
    keep track of move number
    place obstacle in front for each move (4665 moves from part A)
    change map of visited to include direction
    is a loop if the x, y and direction are already in the map
  */
